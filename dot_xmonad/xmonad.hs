...
floats =
  renamed [Replace "floats"] $ windowNavigation $ limitWindows 20 simplestFloat
...
myLayoutHook =
  ...
  where
    -- I've commented out the layouts I don't use.
        myDefaultLayout = grid ||| floats ||| tall
...
myKeys =
  ...
  , -- Exoand vert window width
    ...
    ("C-e e", spawn (myEmacs ++ ("--eval '(dashboard-refresh-buffer)'")))
  , -- start emacs
  ...
  , -- dired emacs file manager
    ("C-e s", spawn (myEmacs ++ ("--eval '(+eshell/here)'")))
  ...
  xmonad
    $                 ewmh def
          ...
          -- "xmonadctl 0" to generate full list of commands written to ~/.xsession-errors.
                        ...
                        , logHook = workspaceHistoryHook <+> myLogHook <+> dynamicLogWithPP
                                      xmobarPP
